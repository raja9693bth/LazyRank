...
| **Notification Preferences** | `notification_preferences` | `id`, `email`, `name`, `notify_on_outranked`, `notify_on_nomination`, `created_at` | **Internal** | Stored preferences. Customer-facing email delivery is inactive until dedicated delivery infrastructure is configured. |
